...
All skills inspect the target's resolved versions and configuration before prescribing APIs. Bundled examples target **5.3.0**; other releases require verification against their published declarations and versioned source. The lifecycle skill also covers the shared Pulumi platform used by the wasmCloud extension and kube, with platform versions checked separately from the application baseline. Detailed task guidance links to the framework docs rather than maintaining another API manual. Planned workflows are tracked through [docs issue 12](https://github.com/di-framework/docs/issues/12) and are added only after implementation and documentation ship.
